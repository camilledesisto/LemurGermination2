...
mean(na.omit(comatsa_seeds2$L))
sd(na.omit(comatsa_seeds2$L))
mean(na.omit(germ$Length))
sd(na.omit(germ$Length))

t.test(comatsa_seeds2$L, germ$Length)
...
germ_time2 <- list(
  G = list(
    G1 = list(V = 1, nu = 0.002),  # Prior for 'animal' (phylogenetic random effect)
    G2 = list(V = 1, nu = 0.002),  # Prior for 'species'
    G3 = list(V = 1, nu = 0.002)   # Prior for 'Lemur_Name'
  ),
  R = list(
    V = 1, nu = 0.002               # Prior for the residual variance
  )
)

germ_time$animal <- germ_time$Lemur_Species
germ_time$animal[germ_time$animal=="L. catta"] <- "Lemur_catta"
germ_time$animal[germ_time$animal=="E. mongoz"] <- "Eulemur_mongoz"
germ_time$animal[germ_time$animal=="E. flavifrons"] <- "Eulemur_flavifrons"
germ_time$animal[germ_time$animal=="V. rubra"] <- "Varecia_rubra"
germ_time$animal[germ_time$animal=="E. coronatus"] <- "Eulemur_coronatus"
germ_time$animal[germ_time$animal=="V. variegata"] <- "Varecia_variegata"
germ_time$animal[germ_time$animal=="M. murinus"] <- "Microcebus_murinus"
germ_time$animal[germ_time$animal=="C. medius"] <- "Cheirogaleus_medius"
germ_time_filtered <- germ_time[germ_time$animal!="*Control",] 
germ_time_filtered <- germ_time_filtered[!is.na(germ_time_filtered$animal),]
...
prior_t2 <- list(
  ...
  R = list(
    V = 1, nu = 0.002               # Prior for residual variance (for gaussian family)
  )
)

treatment_phylo5 <- MCMCglmm(
  log(GerminationTime) ~ Treatment, 
  random = ~ animal + species + Name, 
  family = "gaussian",
  ginverse = ginverse_list, 
...
animal_variance_samples <- treatment_phylo5$VCV[,"animal"]
residual_variance_samples <- treatment_phylo5$VCV[,"units"]
lambda_samples <- animal_variance_samples / (animal_variance_samples + residual_variance_samples)
lambda_CI <- HPDinterval(as.mcmc(lambda_samples), prob = 0.95)
lambda_CI
lambda_animal <- animal_variance / total_variance
...
ggarrange(treatment_plot, treatment_plotTime, labels = c("a", "b"), ncol = 1, nrow=2)
...
ggarrange(species_plot, species_plot_time,labels = c("a", "b"), ncol = 1, nrow=2)
...
ggarrange(treatment_plot_wild, treatment_plotTime_wild,labels = c("a", "b", "c"), ncol = 1, nrow=3)
